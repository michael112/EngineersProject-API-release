    ...
    public void testGetCreatingCourseData() throws Exception {
        ...
        this.mockMvc.perform(get(this.testedClassURI + '/' + AdminCourseControllerUrlConstants.AVAILABLE_CREATING_COURSE_DATA)
            ...
            .andExpect(content().contentType("application/json;charset=utf-8"))
                ...
                .andExpect(jsonPath("$.courseLevels.levels[0].name", is(this.testEnvironment.getCourseLevels().get(0).getName())))
                .andExpect(jsonPath("$.courseLevels.levels[1].name", is(this.testEnvironment.getCourseLevels().get(1).getName())))
                .andExpect(jsonPath("$.courseLevels.levels[2].name", is(this.testEnvironment.getCourseLevels().get(2).getName())))
                .andExpect(jsonPath("$.courseLevels.levels[3].name", is(this.testEnvironment.getCourseLevels().get(3).getName())))
                .andExpect(jsonPath("$.courseLevels.levels[4].name", is(this.testEnvironment.getCourseLevels().get(4).getName())))
                .andExpect(jsonPath("$.courseLevels.levels[5].name", is(this.testEnvironment.getCourseLevels().get(5).getName())))

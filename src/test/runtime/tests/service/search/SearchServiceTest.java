...
public class SearchServiceTest extends AbstractRuntimeTest {
    ...
    private Set<CourseSignupJson> getTestSeekCoursesExpectedResult() {
        ...
        CourseSignupJson result = new CourseSignupJson(course.getId(), course.getLanguage().getId(), course.getLanguage().getLanguageName("en"), course.getCourseLevel().getId(), course.getCourseLevel().getName(), course.getCourseType().getId(), course.getCourseType().getCourseTypeName("en"), course.getStudents().size(), course.getPrice());
